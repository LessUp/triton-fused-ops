...
def rope_kernel(
    ...
    for head_idx in range(num_heads):
        ...
        for i in range(0, half_head, BLOCK_SIZE):
            ...
            out1 = x1.to(tl.float32) * cos_val.to(tl.float32) - x2.to(tl.float32) * sin_val.to(tl.float32)
            out2 = x1.to(tl.float32) * sin_val.to(tl.float32) + x2.to(tl.float32) * cos_val.to(tl.float32)
