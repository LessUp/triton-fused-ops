# Project Standardization - Delta Specification
...
**THEN** the README SHALL display project badges (CI, Python version, License)
**AND** SHALL provide both English and Chinese descriptions

**WHEN** a user reads the README
**THEN** the README SHALL include installation instructions for multiple methods
**AND** SHALL include executable code examples
...
**WHEN** a contributor views the repository
**THEN** they SHALL find CONTRIBUTING.md with development setup instructions
**AND** SHALL find CODE_OF_CONDUCT.md with community guidelines

**WHEN** a contributor creates an issue or PR
**THEN** GitHub SHALL present appropriate templates
...
**WHEN** a PR is opened
**THEN** CI SHALL run lint, type-check, and test jobs
**AND** SHALL block merge if any job fails

**WHEN** CI runs tests
**THEN** it SHALL test on Python 3.9, 3.10, and 3.11
**AND** SHALL generate coverage reports
...
**WHEN** running ruff lint
**THEN** all code SHALL pass without errors
**AND** code SHALL be consistently formatted

**WHEN** running mypy type check
**THEN** all public APIs SHALL have complete type annotations
...
**THEN** pyproject.toml SHALL contain all required metadata
**AND** package SHALL include README and LICENSE

**WHEN** importing the package
**THEN** it SHALL import without errors
**AND** SHALL expose __version__ attribute
...
**WHEN** a user runs an example script
**THEN** the script SHALL execute without errors
**AND** SHALL produce meaningful output demonstrating the feature
