...
`triton_ops.__init__` 是主要用户入口，导出 kernel、模块封装、量化 helper、benchmark 类、自动调优工具、dataclass、异常类型，以及 `PerformanceProfile`。
...
`triton_ops.compute` 提供与 Triton kernel 数学上等价的纯 NumPy 实现。无需 GPU 即可在 CPU 上测试 — 用作正确性参考和测试目标。
